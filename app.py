...
# ──────────────────────────────────────────────────────────────
st.set_page_config(page_title="3학년 학부모 상담 예약", page_icon="🗓️", layout="centered")
...
st.markdown("""
<style>
:root{ --pri:#2563EB; --pri-d:#1E40AF; --ink:#111827; --line:#E5E7EB; }
...
.hero{ background:var(--pri); color:#fff; border-radius:16px; padding:22px 20px; }
.hero h1{ font-size:1.45rem; font-weight:800; margin:0 0 6px; }
.hero p{ margin:0; opacity:.95; font-size:.95rem; line-height:1.55; }
.card{ border:1px solid var(--line); border-radius:14px; padding:18px; background:#fff; }
.notice{ background:#F3F6FF; border:1px solid #DCE5FF; border-radius:12px;
         padding:12px 14px; font-size:.9rem; color:#1E3A8A; line-height:1.6; }
.summary b{ color:var(--pri-d); }
.bigok{ background:#ECFDF5; border:1px solid #A7F3D0; border-radius:14px; padding:18px;
        color:#065F46; font-size:1.02rem; line-height:1.7; }
.warnbox{ background:#FFF7ED; border:1px solid #FED7AA; border-radius:14px; padding:18px;
          color:#9A3412; font-size:1rem; line-height:1.7; }
...
.cell-time{ display:flex;align-items:center;justify-content:center;height:42px;
            font-weight:700;font-size:.85rem;color:var(--ink);background:#F9FAFB;border-radius:8px; }
.cell-head{ display:flex;align-items:center;justify-content:center;height:38px;
            font-weight:700;font-size:.82rem;color:#fff;background:var(--pri);border-radius:8px; }
.cell-head.time{ background:#374151; }
.cell-empty{ height:42px;border-radius:8px;background:#F3F4F6; }
.cell-block{ display:flex;align-items:center;justify-content:center;height:42px;
             font-size:.78rem;font-weight:600;color:#6B7280;background:#E5E7EB;border-radius:8px; }
.stButton>button{ width:100%; border-radius:8px; height:42px; padding:0 2px;
                  font-size:.82rem; font-weight:700; line-height:1.15;
                  border:1px solid var(--pri); color:#fff; background:var(--pri); }
.stButton>button:hover{ background:var(--pri-d); border-color:var(--pri-d); color:#fff; }
.stButton>button:disabled{ background:#F3F4F6; color:#B0B5BD; border:1px dashed #D1D5DB; }
...
def view_intro():
    ...
    <div class="card">
      <b>📌 신청 안내</b>
      <ul style="margin:10px 0 0; padding-left:18px; line-height:1.85; font-size:.93rem;">
...
def view_calendar():
    ...
    st.markdown('<div class="notice">파란 칸을 누르면 신청됩니다. '
                '회색 점선 칸은 <b>마감</b>되어 선택할 수 없습니다.</div>', unsafe_allow_html=True)
