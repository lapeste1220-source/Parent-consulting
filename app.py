...
기능
 ...
 - ★ 1차/2차 구분 없이 2주 전체 타임테이블을 한 화면에 표시, 그중 한 칸만 선택
...
PHASES = {
    ...
    "2차": {
        ...
        "cells": {
            ...
            ("7/31(금)", "13:00"): {"teachers": ["정재성"]},
            ("7/31(금)", "14:00"): {"teachers": ["정재성"]},
            ("7/31(금)", "15:00"): {"teachers": ["정재성"]},
            ("7/31(금)", "16:00"): {"teachers": ["정재성"]},
...
.weeklabel{ font-weight:800; font-size:1.02rem; color:var(--pri-d) !important;
...
def view_intro():
    ...
    <div class="hero">
      ...
      <p>여름방학 2주간 학부모 상담 신청 페이지입니다.<br>
    ...
    <div class="card">
      ...
      <ul>
        ...
        <li>2주간의 전체 일정에서 <b>원하는 한 칸</b>을 누르면 신청됩니다. <b>선착순</b>이며 마감된 칸은 선택할 수 없습니다.</li>
...
def render_week(phase, pdata, vis, taken):
    """한 주(차수) 타임테이블을 그린다. 칸을 누르면 confirm 으로 이동."""
    dates = pdata["dates"]; cells = pdata["cells"]
    st.markdown(f'<div class="weeklabel">{pdata["title"]}</div>', unsafe_allow_html=True)
            ...
            with col:
                for tc in shown:
                    ...
                    if st.button(label, key="b_" + key, disabled=is_taken, use_container_width=True):
                        ss.sel = {"phase": phase, "date": d, "time": rt, "teacher": tc, **ss.info}
...
def view_calendar():
    ...
    st.write("")
    st.markdown('<div class="notice">2주간의 전체 일정입니다. '
                '원하는 <b>한 칸</b>을 누르면 신청됩니다. '
    ...
    # ★ 1차/2차 구분(라디오) 없이 두 주를 한 화면에 모두 표시
    for phase, pdata in PHASES.items():
        render_week(phase, pdata, vis, taken)
